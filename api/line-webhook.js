...
//
// 【今回の変更：連携完了メッセージに本人確認の案内を追加（9/1）】
//   これまでの文面は「連携が完了しました！これからマッチ通知などをお届けします。」
//   だけだったため、登録がすべて終わったように読めていた。
//   実際には No.145「登録の順序を『本人確認→LINE連携』から『LINE連携→本人確認』へ
//   入れ替え」により、LINE連携のあとに本人確認が残っている。
//   次に何をすればよいかが書かれておらず、ここで手が止まる導線上の欠陥だった。
//   そこで、①次にやること（本人確認）を1つだけ示す ②なぜ必要か（承認まで
//   募集・マッチングができない）を添える ③リンクの先が本人確認画面であることを
//   明示する、の3点を追加した。
//   リンク先の分岐はコード側ではなくアプリ側（afterLineLinked）が行っており、
//   本人確認が未提出なら本人確認画面、審査中なら審査中画面、承認済みならトップへ
//   自動で進む。したがってURLは変えていない。
//   「すでに連携済み」のときの返信は変更していない。その利用者が本人確認まで
//   終えているかどうかをこの時点で判定していないため、断定した案内をすると
//   承認済みの人に誤った指示を出すことになる。
        ...
        if (/^\d{6}$/.test(text)) {
          ...
          await replyMessage(
            ...
            PRE_LAUNCH
              ...
              : '連携が完了しました！\n\n' +
                'つづいて「本人確認」をお願いします。\n' +
                '生年月日を確認できる身分証のご提出が必要です。運営の確認が完了するまで、募集・マッチングはご利用いただけません。\n\n' +
                '▼ 下のリンクを押すと、本人確認の画面に進みます\n' +
                APP_URL + '\n\n' +
                'お相手が見つかったときは、このLINEに通知が届きます。'
          );
        }
      }
    }
